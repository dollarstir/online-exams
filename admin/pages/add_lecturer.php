...
if ($result->num_rows > 0) {
	...
	$sph = $row['phone'];
	if ($sem == $email) {
	 header("location:../lecturer.php?rp=1189");	
	}else{
	
	if ($sph == $phone) {
	 header("location:../lecturer.php?rp=2074");	
	}else{
...
VALUES ('$student_id', '$fname', '$lname', '$gender', '$dob', '$address', '$email', '$phone', '$department', '$category','lecturer')";

if ($conn->query($sql) === TRUE) {
  header("location:../lecturer.php?rp=6310");
} else {
  header("location:../lecturer.php?rp=9157");
}
